...
namespace big
{
	...
	Vehicle persist_car::load_vehicle(std::string file_name)
	{
		...
		return spawn_vehicle_full(vehicle_json, g_local_player.player_ped);
	}
	...
	Vehicle persist_car::spawn_vehicle_json(nlohmann::json vehicle_json, Ped ped)
	{
		...
		auto pos = ENTITY::GET_ENTITY_COORDS(g_local_player.player_ped, true);
		...
		if (VEHICLE::IS_THIS_MODEL_A_CAR(ENTITY::GET_ENTITY_MODEL(vehicle)) == TRUE || VEHICLE::IS_THIS_MODEL_A_BIKE(ENTITY::GET_ENTITY_MODEL(vehicle)))
		{
			...
			VEHICLE::_SET_VEHICLE_INTERIOR_COLOR(vehicle, vehicle_json["Interior Color"]);
			VEHICLE::_SET_VEHICLE_DASHBOARD_COLOR(vehicle, vehicle_json["Dash Color"]);
			if (vehicle_json["Clan Logo"] == TRUE)
				vehicle_helper::add_clan_logo_to_vehicle(vehicle, ped);
			VEHICLE::_SET_VEHICLE_XENON_LIGHTS_COLOR(vehicle, vehicle_json["Headlight Color"]);
		}
		return vehicle;
	}
	...
	nlohmann::json persist_car::get_vehicle_json(Vehicle vehicle)
	{
		...
		if (VEHICLE::IS_THIS_MODEL_A_CAR(ENTITY::GET_ENTITY_MODEL(vehicle)) == TRUE || VEHICLE::IS_THIS_MODEL_A_BIKE(ENTITY::GET_ENTITY_MODEL(vehicle)))
		{
			...
			VEHICLE::_GET_VEHICLE_INTERIOR_COLOR(vehicle, &interior_color);
			VEHICLE::_GET_VEHICLE_DASHBOARD_COLOR(vehicle, &dashboard_color);
			...
			vehicle_json["Headlight Color"] = VEHICLE::_GET_VEHICLE_XENON_LIGHTS_COLOR(vehicle);
		}
		return vehicle_json;
	}
	...
	{
		Vehicle vehicle = PED::GET_VEHICLE_PED_IS_IN(g_local_player.player_ped, false);
